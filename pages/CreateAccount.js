...
class CreateAccount extends React.Component {
  ...
  validateInputs(username, email, password, passwordConfirm) {
    if (username === "") {
      ...
      this.setState({
        errorMessage: (
          <p className={styles.error}>Please Enter and Confirm your Password</p>
        ),
      });
  ...
  async onCreateAccount() {
    ...
    if (response.ok) {
      ...
      const queryParams = {
        //this has got to be fetched from somewhere rather than statically kept here
      ...
      window.location.replace(
        `https://accounts.spotify.com/authorize?${paramString}`
      );
    } else {
  ...
  render() {
    return (
      <div className={styles.main}>
        ...
        <br />
        <button className={styles.createAccount} onClick={this.onCreateAccount}>
          Create Account
        </button>
      </div>
    );
